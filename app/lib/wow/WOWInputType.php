...
class WOWInputType
{
	...
	function WOWInputType(& $_ctrl, $_useacknowledge=TRUE)
	{
		...
		if($_useacknowledge){
			$this->acknowledge = array('', '非公開', '承認待ち', '公開');
		}else{
...
//--------------------------------------------------------------------------------
	...
	{
		$column = $_labelType;
		if(isset($this->dataObject[$column])){
	...
	function _getEditItem_acknowledge($_labelHead, $_labelIndex, $_labelType, $_hidden=FALSE)
	{
		...
		$html = '';
		foreach($items as $i){
			$html .= "<input type=\"radio\" id=\"{$column}{$i['id']}\" name=\"{$column}\" value=\"{$i['id']}\" " . (($i['id'] == $this->dataObject[$column]) ? 'checked="checked" ' : '') . "/>
			<label for=\"{$column}{$i['id']}\" class=\"" . (isset($this->errors[$column]) ? ' error-message' : '') . "\"" . (($_hidden) ? "\" style=\"display: none;" : '') . ">
			<span class=\"w30\">" . htmlspecialchars($i['label_text'], ENT_QUOTES) . "</span></label>";
		}
		return $html;
	}
	/*
	...
	function _getEditItem_check($_labelHead, $_labelIndex, $_labelType, $_hidden=FALSE)
	{
		...
		foreach($items as $k => $i){
			...
			$html .= "<input type=\"checkbox\" id=\"{$column}{$k}\" name=\"{$column}[]\" value=\"{$k}\" " . ((1 << ($k-1) & $this->dataObject[$column]) ? 'checked="checked" ' : '') . "/>
			<label for=\"{$column}{$k}\" class=\"" . (isset($this->errors[$column]) ? ' error-message' : '') . "\"" . (($_hidden) ? "\" style=\"display: none;" : '') . "><span class=\"w30\">" . htmlspecialchars($i, ENT_QUOTES) . "</span></label>";
		}
		return $html;
	}
...
//			$date = date('Y-m-d');
	...
	function _getEditItem_datetime($_labelHead, $_labelIndex, $_labelType, $_hidden=FALSE)
	{
		...
		foreach($options as $o){
			...
			if(substr($o, 0, 5) === substr($time, 0, 5)){
				$isSelected = ' selected="selected"';
				$select .= "<option value=\"{$time}\"{$isSelected}>" . substr($o, 0, 5) . "</option>";
			}else{
				$select .= "<option value=\"{$o}\">" . substr($o, 0, 5) . "</option>";
			}
		}
		$select .= '</select>';
	...
	function _getViewItem_file($_labelHead, $_labelIndex, $_labelType)
	{
		...
		if(!isset($this->dataObject[$column])){
			...
			return '<a href="../f.php?id=' . htmlspecialchars($this->dataObject[$column], ENT_QUOTES) . '" title="画像" class="cboxaction cboxElement"><img src="../f.php?id=' . htmlspecialchars($this->dataObject[$column], ENT_QUOTES) . '&mode=icon" width="120" height="120"/></a>';
		}
	}
	function _getEditItem_file($_labelHead, $_labelIndex, $_labelType, $_hidden=FALSE)
	{
		...
		$html = "<div class=\"file-upload\"" . (($this->dataObject[$column]) ? ' style="display: none;"' : ''). "><a title=\"画像管理\" href=\"./filewindow.php?action=edit&amp;dir={$column}\" id=\"{$column}_a\" class=\"cboxaction\"><img height=\"21\" border=\"0\" width=\"137\" class=\"rollover\" alt=\"\" src=\"common/img/upload_btn.gif\"/></a></div>";
	...
	function _getEditItem_iradio($_labelHead, $_labelIndex, $_labelType, $_hidden=FALSE)
	{
		...
		//}
		foreach($masters as $k => $i){
			if($i){
				$html .= "<input type=\"radio\" id=\"{$column}{$k}\" name=\"{$column}\" value=\"{$k}\" " . (($k == $this->dataObject[$column]) ? 'checked="checked" ' : '') . "/>
<label for=\"{$column}{$k}\" class=\"" . (isset($this->errors[$column]) ? ' error-message' : '') . "\"" . (($_hidden) ? "\" style=\"display: none;" : '') . ">
<span class=\"w30\">" . htmlspecialchars($i, ENT_QUOTES) . "</span></label>";
			}
		}
...
<span class=\"w30\">" . htmlspecialchars($i, ENT_QUOTES) . "</span></label>";
	...
	function _getEditItem_richtext($_labelHead, $_labelIndex, $_labelType, $_hidden=FALSE)
	{
		...
		return	"<textarea id=\"{$column}\" name=\"{$column}\" rows=\"12\" cols=\"60\" class=\"w640 ckeditor" . (isset($this->errorList[$column]) ? ' error' : '') . (($_hidden) ? "\" style=\"display: none;" : '') . "\">" . htmlspecialchars($this->dataObject[$column], ENT_QUOTES) . "</textarea>";
	}
	/*
	 * _select : セレクトオプション
	 */
...
//		if(preg_match('/^(\w+)(\d+)$/', $_labelHead, $matches)){
	...
	{
		$column = $_labelHead . $_labelIndex . '_' . $_labelType;
		return "<textarea id=\"{$column}\" name=\"{$column}\" rows=\"4\" cols=\"60\" class=\"w640h200" . (!$_hidden && isset($this->errorList[$column]) ? ' error' : '') . (($_hidden) ? "\" style=\"display: none;" : '') . "\">" . htmlspecialchars($this->dataObject[$column], ENT_QUOTES) . "</textarea>";
	}
	/*
	 * _json : JSONデータ
	 *	   jquery.json-viewer
	 */
	function _getViewItem_json($_labelHead, $_labelIndex, $_labelType)
	{
		$column = $_labelHead . $_labelIndex . '_' . $_labelType;
		return nl2br(htmlspecialchars($this->dataObject[$column], ENT_QUOTES));
	}
	function _getEditItem_json($_labelHead, $_labelIndex, $_labelType, $_hidden=FALSE)
	{
		$column = $_labelHead . $_labelIndex . '_' . $_labelType;
		return "<textarea name=\"{$column}\" class=\"json-input" . (!$_hidden && isset($this->errorList[$column]) ? ' error' : '') . "\" style=\"display: none;" . "\">" . htmlspecialchars($this->dataObject[$column], ENT_QUOTES) . "</textarea>" .
		"<pre id=\"{$column}\" class=\"json-output\"></pre>";
	 ...
	 */
	function _createTimeList($interval='1m')
	{
