...
/**
 */
class WOWOtherValidator extends Validator
{
	function validateDoneEdit($values)
	{
		...
		// else{
		// 	$db = $this->_useDb();
		// 	$label_result = $db->query("SELECT id FROM `other` WHERE label_text = '" . $values['label_text'] . "' AND delete_datetime IS NULL AND acknowledge = '3'");
		...
		if(!$this->isEmpty($values['recommenda_text'])){
			...
			$db = $this->_useDb();
			$result =& $db->query("SELECT id FROM `other` WHERE label_text = '" . $values['recommenda_text'] . "' AND delete_datetime IS NULL AND acknowledge = '3'");
		...
		if(!$this->isEmpty($values['recommendb_text'])){
			//入力されている場合、商品名が登録されているかチェック
			$db = $this->_useDb();
			$result =& $db->query("SELECT id FROM `other` WHERE label_text = '" . $values['recommendb_text'] . "' AND delete_datetime IS NULL AND acknowledge = '3'");
			$row =& $result->fetchRow(MDB2_FETCHMODE_ASSOC);
		...
		if(!$this->isEmpty($values['recommendc_text'])){
			//入力されている場合、商品名が登録されているかチェック
			$db = $this->_useDb();
			$result =& $db->query("SELECT id FROM `other` WHERE label_text = '" . $values['recommendc_text'] . "' AND delete_datetime IS NULL AND acknowledge = '3'");
			$row =& $result->fetchRow(MDB2_FETCHMODE_ASSOC);
		...
		if(!$this->isEmpty($values['recommendd_text'])){
			//入力されている場合、商品名が登録されているかチェック
			$db = $this->_useDb();
			$result =& $db->query("SELECT id FROM `other` WHERE label_text = '" . $values['recommendd_text'] . "' AND delete_datetime IS NULL AND acknowledge = '3'");
			$row =& $result->fetchRow(MDB2_FETCHMODE_ASSOC);
